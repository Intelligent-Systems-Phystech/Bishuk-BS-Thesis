...
class SEIRSNetworkModel():
	...
	Params: 
			...
			initQ_R         Initial number of isolated recovered individuals
							(all remaining nodes initialized susceptible)   
	"""
	def __init__(self, G, beta, sigma, gamma, X_init = None,
		...
		if isinstance(X_init, numpy.ndarray):
			initE   = len(numpy.where(X_init == 2)[0])
			initI   = len(numpy.where(X_init == 3)[0])
			initR   = len(numpy.where(X_init == 4)[0])
			initF   = len(numpy.where(X_init == 5)[0])
			initQ_E = len(numpy.where(X_init == 6)[0])
			initQ_I = len(numpy.where(X_init == 7)[0])
		...
		if not isinstance(X_init, numpy.ndarray):
			...
			self.numE[0]        = len(numpy.where(X_init == 2)[0])
			self.numI[0]        = len(numpy.where(X_init == 3)[0])
			self.numR[0]        = len(numpy.where(X_init == 4)[0])
			self.numF[0]        = len(numpy.where(X_init == 5)[0])
			self.numQ_E[0]      = len(numpy.where(X_init == 6)[0])
			self.numQ_I[0]      = len(numpy.where(X_init == 7)[0])
			self.numS[0]        = (self.numNodes - self.numE[0] - self.numI[0] - self.numR[0] - self.numQ_E[0] - self.numQ_I[0] - self.numF[0])
			self.N[0]           = self.numNodes - self.numF[0]
		...
		if not isinstance(X_init, numpy.ndarray):
			...
			self.X = numpy.array([X_init]).reshape(-1,1)
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^     
	...
	def run_iteration(self, dtau = None):
...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	def run(self, T, checkpoints=None, print_interval=10, verbose='t', num_sample_in_T = None):
		if(T > 0):
			self.tmax += T
		else:
			return False
		...
		if num_sample_in_T:
			if isinstance(num_sample_in_T, int):
				if num_sample_in_T <= 0:
					raise TypeError(f"num_sample_in_T must be over 0, but now equal {num_sample_in_T}")
				
				dtau = T / num_sample_in_T

			else:
				raise TypeError(f"num_sample_in_T must be int, but received {type(num_sample_in_T)}")

		while running:
			if not dtau:
				running = self.run_iteration()
			else:
				running = self.run_iteration(dtau = dtau)
			#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
